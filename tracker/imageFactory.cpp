...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string, SDL_Surface*>::iterator ptr_s = surfaces.begin();
  while (ptr_s != surfaces.end()) {
    SDL_FreeSurface(ptr_s -> second);
    ++ptr_s;
  }

  //for(auto& ti : textures) SDL_DestroyTexture(ti.second);
  std::map<std::string, SDL_Texture*>::iterator ptr_t = textures.begin();
  while (ptr_t != textures.end()) {
    SDL_DestroyTexture(ptr_t -> second);
    ++ptr_t;
  }
  ...
  //for(auto& fi : images  ) {
    ...
    //delete fi.second;
  //}
  std::map<std::string, Image*>::iterator ptr_i = images.begin();
  while (ptr_i != images.end()) {
    std::cout <<"deleting " << ptr_i -> first << std::endl;
    delete ptr_i -> second;
    ++ptr_i;
  }
...
Image* ImageFactory::getImage(const std::string& name) {
  ...
  if ( it == images.end() ) {
    ...
    if ( transparency ) {
      int keyColor = SDL_MapRGBA(surface->format, 255, 255, 255, 255/2);
...
std::vector<Image*> ImageFactory::getFolderImages(const std::string& name) {
  std::map<std::string, std::vector<Image*> >::const_iterator 
    pos = multiImages.find(name); 
  if ( pos != multiImages.end() ) {
    return pos->second;
  ...
  std::string sheetPath = gdata.getXmlStr(name+"/folder");    
  for (unsigned i = 0; i < numberOfFrames; i++) {
    std::stringstream ss;
    ss << sheetPath << "/" << i << ".png";
